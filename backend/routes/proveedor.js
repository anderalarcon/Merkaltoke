...
router.route("/create").post(async (req, res) => {
    try {
      ...
      const newProveedor = await pool.query(
        ...
        [ nombre_proveedor, ruc, razon_social, utilidades, id_cuenta_proveedor, id_local ]
...
router.route("/update/:id_proveedor").put(async (req, res) => {
    try {
      ...
      const proveedores = await pool.query(
        ...
        [ nombre_proveedor, ruc, razon_social, utilidades, id_cuenta_cliente, id_local, id_proveedor  ]
      );
      res.status(200).json({
        status:"success",
        data:{proveedores:proveedores.rows[0]},
...
router.route("/delete/:id_proveedor").delete(async (req, res) => {
    try {
      const { id_proveedor } = req.params;
      const proveedorAeliminar = await pool.query("DElETE FROM tbl_proveedor WHERE id_proveedor=$1", [
        id_proveedor,
      ]);
